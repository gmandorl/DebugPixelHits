...
class DebugPixelHits_MC_CluRef : public edm::one::EDAnalyzer<edm::one::SharedResources> {
    public:
        explicit DebugPixelHits_MC_CluRef(const edm::ParameterSet&);
        ~DebugPixelHits_MC_CluRef();
...
//
DebugPixelHits_MC_CluRef::DebugPixelHits_MC_CluRef(const edm::ParameterSet& iConfig):
...
DebugPixelHits_MC_CluRef::~DebugPixelHits_MC_CluRef()
{
}

    void
DebugPixelHits_MC_CluRef::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    using namespace edm;
    run_  = iEvent.id().run();
...
DEFINE_FWK_MODULE(DebugPixelHits_MC_CluRef);
